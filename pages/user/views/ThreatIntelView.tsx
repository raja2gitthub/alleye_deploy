...
const ThreatIntelView: React.FC<ThreatIntelViewProps> = ({ updates, onRead }) => {
    ...
    return (
        <div className="animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {updates.map(item => (
                    <Card key={item.id} className="!p-0 flex flex-col group cursor-pointer overflow-hidden transform hover:-translate-y-1 transition-transform duration-300" onClick={() => handleRead(item)}>
                        ...
                        <div className="p-4 flex-1 flex flex-col">
                            <p className="text-sm text-text-secondary">By {item.author?.name || 'Alleye'}</p>
